...
ADMINS_ID, TELEGRAM_TOKEN, CLIENT_TOKEN, PANEL_URL, SUB_URL, LANG, PANEL_ADMIN_ID = None, None, None, None, None, None, None
...
def set_config_variables(configs, server_url):
    ...
    global ADMINS_ID, TELEGRAM_TOKEN, PANEL_URL, SUB_URL, LANG, PANEL_ADMIN_ID, CLIENT_TOKEN
    ...
    # در صورتی که لینک ساب در دیتابیس نبود، همان لینک پنل را قرار بده تا برنامه کرش نکند
    SUB_URL = configs.get("sub_base_url", server_url)
...
def panel_url_validator(url):
    ...
    print(colored("URL format is valid!", "green"))
    return url
...
def set_by_user():
    ...
    print()
    print(colored(
        "Example: https://panel.example.com/7frgemkvtE0/ADMIN_UUID\n[Enter Admin Panel URL for API connection]",
        "yellow"))
    while True:
        url = input("[+] Enter your Admin Panel URL:")
        if not url:
    ...
    print()
    print(colored(
        "Example: https://sub.example.com/7frgemkvtE0/\n[Enter Subscription Base URL for your users]",
        "yellow"))
    while True:
        sub_url = input("[+] Enter your Subscription Base URL:")
        if not sub_url:
            print(colored("Subscription URL is required!", "red"))
            continue
        sub_url = panel_url_validator(sub_url)
        if not sub_url:
            continue
        break
    ...
    return admin_ids, token, url, sub_url, lang, client_token


def set_config_in_db(db, admin_ids, token, url, sub_url, lang, client_token):
    try:
        ...
        if not db.select_str_config():
            ...
            db.add_str_config("sub_base_url", value=sub_url)
        else:
            ...
            # Check if sub_base_url key exists, then update or add
            configs = db.select_str_config()
            keys = [conf['key'] for conf in configs]
            if "sub_base_url" in keys:
                db.edit_str_config("sub_base_url", value=sub_url)
            else:
                db.add_str_config("sub_base_url", value=sub_url)
...
def print_current_conf(conf, server_url):
    ...
    print(f"[+] Admin Panel URL: {server_url}")
    print(f"[+] Subscription URL: {conf.get('sub_base_url', 'Not Set')}")
...
if __name__ == '__main__':
    ...
    if conf.get('bot_admin_id') and conf.get('bot_token_admin') and conf.get('bot_lang') and server_url:
        ...
        if input().lower() == "y":
            admin_ids, token, url, sub_url, lang, client_token = set_by_user()
            set_config_in_db(db, admin_ids, token, url, sub_url, lang, client_token)
            conf = load_config(db)
            server_url = load_server_url(db)
...
# These lines run when config.py is imported in other modules
db = UserDBManager(USERS_DB_LOC)
db.set_default_configs()
conf = load_config(db)
server_url = load_server_url(db)
